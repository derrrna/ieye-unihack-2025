            ...
            <div className="p-15 text-[#1F100A] flex flex-col justify-center w-full m-7 sm:w-[500px] h-3/5 lg:h-3/4 bg-[#FFCF32] rounded-4xl shadow-md font-[Dongle] lg:text-4xl">
                <form onSubmit={handleLogin}>
                    ...
                    <div className='mt-5 flex flex-col'>
                        ...
                        <select name="name" className='mb-3 bg-black rounded-2xl text-2xl pl-2 text-white'
                        onChange={(e) => setId(e.target.value)}>
                            {users.map(user =>
                                <option value={user.id}>{user.name}</option>
                            )}
                        </select>
                        <button type="submit" class="btn bg-[#5E93E8] border-none text-xl">Select your Name!</button>
                    </div>
                </form>
